...
def _b64decode(data: str) -> bytes:
    ...
    try:
        ...
        # A single truncated/corrupt link must never bubble up as a raw
        # exception: that would abort parsing of every OTHER (perfectly
        # valid) link in the same subscription. Convert it into our own
        # ParseError so callers can just skip this one link.
...
def fingerprint(outbound: dict) -> str:
    """Unique fingerprint for dedup (without tag)."""
...
def _build_tls(params: dict, default_sni: str = "", force: bool = False):
    """Build the tls block from a link's standard query parameters."""
...
def _build_transport(network: str, params: dict, host: str = "", path: str = ""):
    """Build the transport block from the link's network type."""
    network = (network or "tcp").lower()
    if network in ("tcp", "raw", "none", ""):
        header_type = _first(params, "headerType")
        if header_type == "http":  # vmess tcp + http obfs
...
def _parse_vmess(link: str):
    ...
    tparams = {
        ...
        "host": [host],
        "path": [path],
        "serviceName": [path],  # for vmess+grpc, the service name is carried in "path"
    }
...
def parse_link(link: str) -> dict:
    """link -> {fingerprint, name, link, outbound}, or raises ParseError."""
    link = link.strip()
...
def parse_many(links: list) -> tuple[list, int]:
    """list of links -> (valid items, count of invalid ones)."""
    items, bad = [], 0
    seen = set()
    for link in links:
        try:
            ...
            continue
        except Exception as exc:  # noqa: BLE001 - defense in depth: a bug in
            # one protocol's parser must never abort the whole subscription.
            bad += 1
